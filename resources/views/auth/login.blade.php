					...
					<div class="col-md-4 offset-md-4 px-3">
						<div class="box b-a-0">
							<div class="p-2 text-xs-center">
								<h5>خوش آمدید</h5>
								<p>سیستم مدیریتی فروشگاه ملکه ثریا</p>
							</div>
							<form class="form-material mb-1" method="POST" action="{{ route('login') }}">
                    @csrf

								<div class="form-group">
									<input type="text" id="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" id="exampleInputEmail" placeholder="نام کاربری" name="email" value="{{ old('email') }}" required autofocus>
                  @if ($errors->has('email'))
